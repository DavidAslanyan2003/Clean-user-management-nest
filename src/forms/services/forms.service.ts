...
export class FormsService {
  ...
  async postNewsLetterForm(newsLetterDto: NewsLetterDto) {
    ...
    try {
      ...
      await queryRunner.manager.getRepository(NewsLetter).save(newsLetterFinalForm);
  ...
  async postContactUsForm(contactUsDto: ContactUsDto) {
    const queryRunner = this.contactUsRepository.manager.connection.createQueryRunner();
    await queryRunner.connect();
    await queryRunner.startTransaction();
